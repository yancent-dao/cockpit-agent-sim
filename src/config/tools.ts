...
**查天气对比直接用 weather.nearby，不要拿这个的结果去循环调 weather.query**——
那样每个区县各建一张卡，桌面会被刷满；weather.nearby 已经内置了区县枚举，
一次调用就拿到全部区县的天气对比（含坐标，能直接传给 navigation.setDestination）。
这个工具留给天气以外、需要区县名称/坐标列表的场景。`,
    permission: '彩',
    params: {
...
找到后把该地点的 location 传给 navigation.modifyRoute 的 addWaypoint 即可顺路去（终点不动）。`,
  ...
  /* ── L2 应用级：天气（真实接高德） ── */
  {
    ...
    brief: '查城市天气预报',
    fast: true,
    desc: '查询**单个**地点的天气，同时返回实况与未来几天预报。查询成功后天气卡会自动显示在屏幕上，你只负责口头播报重点，不要再建卡。查不到这个地点时会得到 unavailable。'
      + '**注意**：这是气象服务的城市级数据，跟车上传感器（车外温度、当前天况）测的"此刻这里"不是一回事，'
      + '两者对不上很正常，别当成矛盾去质问用户——车里冷不冷、雨大不大以传感器为准。'
      + '**要查"周边/附近区县"这类多个地点时不要循环调用这个**——一个地方一次调用没问题，'
      + '但连续调十几次会把桌面刷满一堆各自独立的天气卡，改用 weather.nearby。',
    permission: '彩',
    params: {
  ...
  },
  {
    ...
    brief: '查周边区县天气一览',
    fast: true,
    desc: '一次性查一个城市下辖所有区县的天气，并发查、只建**一张**对比列表卡（不是每个区县一张）。'
      + '用户说"周边天气怎么样""附近哪个县在下雨"这类多地对比的需求用这个，'
      + '**不要自己先调 region.districts 拿列表再循环调 weather.query**——那样每个区县各建一张卡，桌面会被刷满。'
      + '返回的 data.districts 每项都带坐标，"找附近下雨的县城导航过去"这类需求'
      + '可以直接从这里挑出符合条件的一个，再传给 navigation.setDestination，不用再查一次区县列表。'
      + '卡片上已经是全部区县的实况对比，你只负责口头挑重点（比如哪几个在下雨、温差大不大），不要逐个念。',
    permission: '彩',
    params: {
      area: { type: 'string', desc: '城市名，如"成都"。不传则用车辆当前所在城市' },
    },
  ...
  },
  {
    name: 'travel.create',
    brief: '建长期行程任务',
    desc: '把一次出行建成**长期任务**，跨上下电一直盯着。**它不装行程内容**——' +
      '攻略/线路/日程必须先用 travel.plan 上卡，只调 create 屏上是张空卡。判据三条同时满足才建：' +
      '有未来时间窗（≥明天）、要多步骤准备、有可监控的外部变量（机票/酒店价这类）——' +
      '"今晚订个餐厅"不建。**时机：先给攻略，用户认可了再建**（认可就是授权，' +
      '不要再问"要不要建任务"这种流程性问题）。**建完告知不询问**：' +
      '"机票酒店我盯着，有变化叫你"，用户随时说不用盯了就撤。' +
      '信息不全照建（待定态），缺的随对话补——**不许连环追问**：一轮最多问一个问题，' +
      '日期、人数、提醒价都能之后再补，watch 里直接带上 flight 和 hotel（不设 threshold）。',
    permission: '彩',
    params: {
      ...
      watch: { type: 'array', items: { type: 'object',
        properties: { kind: { type: 'string', enum: ['flight', 'hotel', 'fx', 'news'] },
                      ...
                      stay: { type: 'object', properties: {
                        city: { type: 'string' }, dayFrom: { type: 'number' }, dayTo: { type: 'number' } },
                        required: ['city', 'dayFrom', 'dayTo'] } },
        required: ['kind'] },
        desc: '要盯的项。跨城市行程**每段住宿各一条 hotel**，stay 标段：' +
          '[{"kind":"flight"},{"kind":"hotel","stay":{"city":"曼谷","dayFrom":1,"dayTo":3}},' +
          '{"kind":"hotel","stay":{"city":"芭提雅","dayFrom":4,"dayTo":4}}]。' +
          '不设 threshold 就只跟踪不提醒' },
    },
  ...
  },
  {
    ...
    params: { taskId: { type: 'string', desc: '只看某一个任务。不传看全部' },
      showTrend: { type: 'string',
        desc: '要看完整走势的委托 id——用户点了价格块/说"看看走势"时传，趋势卡上屏' } },
    handler: 'travelList',
  },
  {
    name: 'travel.refresh',
    brief: '立刻查最新价',
    desc: '立即把监控项采一轮，到提醒线的旅行卡上会出决策条。' +
  ...
  },
  {
    ...
    desc: '联网查东西。车里的知识、新闻之外的实时信息、你不确定的事实，都用这个而不是凭记忆答。结果会上屏，用户能回看数字和人名。**每次搜索要 15 秒起步——把问句写全（对象+时间范围+要什么数据）一次问够，别拆成几次搜，用户在车里等不起。**'
      + '**要出调研报告/多方查证的活，不要在这亲自搜**——task.delegate(background:true) 派子 Agent 后台查（它有独立轮次预算，完成自动通知），你把手头能立即交付的先交付掉；你亲自一轮轮搜，用户只能对着"思考中"干等，轮次还会耗尽在查证上。'
      + '返回的答案往往很长，别整段念——挑一两句最关键的说，剩下的让用户看屏幕。每次调用都会花钱，别为了闲聊或者你本来就知道的常识调它。',
    permission: '彩',
