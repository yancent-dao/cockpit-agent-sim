...
找到后把该地点的 location 作为 waypoints 传给 navigation.setDestination 即可顺路去。`,
  ...
  {
    name: 'voice.ask',
    desc: '向用户提问并给出候选项，用于消歧或征求选择——不是二次确认（二次确认走返回的 confirmToken，不用这个）。问题和选项会自动显示成屏上的选择卡（带序号），你不用建卡。**它只管上屏，不会替你说话**——问题得由你在这一轮的回复里亲口问出来（实测有模型调完它就回一句"问题已经念给你了"，结果用户什么也没听到）。**用户是开车的人，只能用说的回答，不能点屏幕**——所以话术里要说"你说第几个就行/告诉我要哪个"，绝不能说"点一下""你选一个点击"。用户的回答会在下一轮对话里出现。',
    permission: '彩',
    params: {
