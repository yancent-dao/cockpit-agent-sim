...
src/cards/     卡片桌面 · 栅格 · 编排器 · 生命周期 · 抢占 · 恢复 · 等位区   < 1000 行（现 913）
...
- **卡片右上角尺寸调节**（2026-08-13 实拍反馈）：每张卡右上角有 −/＋（还有 ✕，导航卡
  没有）。机制在 `desk.step(id, dir)`：按模板允许的尺寸表（`checkSize` 那张表，不是
  LADDER——LADDER 只有七档，2/3/tower/full 这些专用档故意不进去）走一档，按占用单元数
  排序天然把 2/3 接到导航卡尺寸表的最大端；到头按钮该自己置灰（`desk.canStep` 只读查询，
  车机屏拿它决定 disabled，不重算业务逻辑）。**导航卡不再靠代码强制钉成最大**——
  `evictable:false`/左锚定的优先级保护不变（永不被挤下桌），但尺寸交给这个按钮，
  用户随时能缩小看更多桌面
