...
describe('K 轮滑动窗口 + epoch 摘要', () => {
  it('超过 4 轮后旧轮次折叠成摘要头，最近 4 轮保持全文', async () => {
    const { p } = mk()
    for (const s of ['一', '二', '三', '四', '五', '六']) await p.run(`第${s}句`)
    await p.compaction
    const users = p.thread.filter(m => m.role === 'user').map(m => m.content)
    expect(users.length).toBeLessThanOrEqual(4)
    expect(users).toContain('第六句')
  ...
  it('4 轮以内不压缩——短对话不花这笔钱', async () => {
    const { p, fast } = mk()
    for (const s of ['一', '二', '三']) await p.run(`第${s}句`)
    await p.compaction
  ...
  it('压缩不占响应路径：run 返回时话术已出，压缩晚于它', async () => {
    ...
    // run 已全部返回，此刻压缩可能仍在跑——thread 里的用户轮次仍可能 >4
    await p.compaction
    expect(p.thread.filter(m => m.role === 'user').length).toBeLessThanOrEqual(4)
  })
})
...
describe('压缩的健壮性', () => {
  it('压缩失败保持原样，不丢一条消息', async () => {
    ...
    expect(p.thread.filter(m => m.role === 'user')).toHaveLength(5)
  })
